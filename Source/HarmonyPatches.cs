...
namespace ShowHair
{
	...
	public static class Patch_PawnRenderer_RenderPawnInternal
	{
		...
		//gaberad: used to get custom texture
		public static ShowHair.NoClipMode.IHairUtility HairUtility;
		...
		public static void Initialize()
		{
			if (!typesInitialized)
			{
				...
				try
				{
					...
					//gaberad: get the HairUtility based on the UseFallbackTexture setting
					HairUtility = new ShowHair.NoClipMode.HairUtility(Settings.UseFallbackTexture);
				}
				catch (Exception e)
				{
			...
			if (pawn != null && __instance.graphics.headGraphic != null)
			{
				...
				else if ((hideHats || forceShowHair) && hairLoc > 0)
				{
					if (hairLoc > 0.001f)
					{
						...
						//gaberad: if no clip mode is on and not hiding hats, get custom hair texture, otherwise get default texture
						Material mat = Settings.NoClipMode && !hideHats ?
							HairUtility.GetCustomHairMat(pawn, headFacing) :
							__instance.graphics.HairMatAt_NewTemp(headFacing, true);
